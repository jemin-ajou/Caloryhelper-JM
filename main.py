...
async def analyze_food(file: UploadFile = File(...)):
    ...
    try:
        ...
        system_instruction = (
            "당신은 전문 임상 영양사입니다. 사진 속 음식을 분석하여 다음 JSON 형식으로만 응답해 주세요. "
            "절대로 다른 설명글을 붙이지 마세요.\n\n"
            "{\n"
            "  \"dish_name\": \"요리명\",\n"
            ...
            "  ],\n"
            "  \"tips_advice\": [\n"
            "    \"해당 음식의 영양학적 장점 또는 단점을 바탕으로 한 구체적인 건강 조언 1\",\n"
            "    \"해당 음식과 같이 먹으면 좋은 음식 추천 또는 피해야 할 행동 조언 2\",\n"
            "    \"장기적인 식단 관리를 위한 영양 팁 3\"\n"
            "  ]\n"
            "}"
        )
